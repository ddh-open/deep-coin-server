...
import (
	...
	redis2 "devops-http/framework/provider/redis"
	"errors"
...
func NewRedisCache(params ...interface{}) (interface{}, error) {
	...
	if !container.IsBind(contract2.RedisKey) {
		err := container.Bind(&redis2.ProviderRedis{})
		if err != nil {
			return nil, err
		}
	}
	...
	redisService := container.MustMake(contract2.RedisKey).(contract2.RedisService)
	client, err := redisService.GetClient(redis2.WithConfigPath("cache"))
	if err != nil {
		return nil, err
	}
...
func (r *RedisCache) Remember(ctx context.Context, key string, timeout time.Duration, rememberFunc contract2.RememberFunc, obj interface{}) error {
