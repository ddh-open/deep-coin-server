...
func GetBaseConfig(c framework.Container) *contract2.RedisConfig {
	logService := c.MustMake(contract2.LogKey).(contract2.Log)
	config := &contract2.RedisConfig{Options: &redis.Options{}}
...
func WithConfigPath(configPath string) contract2.RedisOption {
	return func(container framework.Container, config *contract2.RedisConfig) error {
...
func WithRedisConfig(f func(options *contract2.RedisConfig)) contract2.RedisOption {
	return func(container framework.Container, config *contract2.RedisConfig) error {
		f(config)
		return nil
	}
