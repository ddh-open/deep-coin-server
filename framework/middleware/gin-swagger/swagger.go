...
func WrapHandler(h *webdav.Handler, confs ...func(c *Config)) gin2.HandlerFunc {
...
func CustomWrapHandler(config *Config, h *webdav.Handler) gin2.HandlerFunc {
...
func DisablingWrapHandler(h *webdav.Handler, envName string) gin2.HandlerFunc {
...
// if specified environment variable passed
func DisablingCustomWrapHandler(config *Config, h *webdav.Handler, envName string) gin2.HandlerFunc {
	eFlag := os.Getenv(envName)
	if eFlag != "" {
		return func(c *gin2.Context) {
			// Simulate behavior when route unspecified and
