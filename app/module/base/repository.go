...
func (r *Repository) Update(data interface{}, where ...interface{}) error {
	return r.db.Where(where[0], where[1:]...).Updates(data).Error
}
...
func (r *Repository) Find(dst interface{}, filter ...interface{}) error {
	return r.db.Where(filter).Order("id desc").Find(dst).Error
}

// List 根据条件获取列表
func (r *Repository) List(dst interface{}, pageSize int64, page int64, filter ...interface{}) error {
	if len(filter) >= 2 {
		return r.db.Where(filter[0], filter[1:]...).Offset(int(page) - 1).Limit(int(pageSize)).Order("id desc").Find(dst).Error
	} else {
		return r.db.Offset(int((page - 1) * pageSize)).Limit(int(pageSize)).Order("id desc").Find(dst).Error
	}
}
