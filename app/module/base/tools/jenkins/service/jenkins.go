...
func (p *JenkinsPlugin) getJenkinsClient(ctx context.Context) (*gojenkins.Jenkins, error) {
	...
	if err != nil {
		base.Logger.Error("create jenkins client err:%v", zap.Error(err))
		return nil, err
	}
...
func (p *JenkinsPlugin) Exec(ctx context.Context) error {
	...
	if err != nil {
		base.Logger.Error("create jenkins client err:", zap.Error(err))
		return err
	}
...
func (p *JenkinsPlugin) afterExec(ctx context.Context, jenkinsClient *gojenkins.Jenkins, queueID int64) error {
	...
	if err != nil {
		base.Logger.Error("get jenkins job err:%v", zap.Error(err))
		return err
	}
	task, err := jenkinsClient.GetQueueItem(ctx, queueID)
	if err != nil {
		base.Logger.Error("get jenkins queue item err:%v", zap.Error(err))
		return err
	}
	...
	for i := 0; i < 30; i++ {
		...
		_, err = task.Poll(ctx)
		if err != nil {
			base.Logger.Error("get jenkins queue poll task err:%v", zap.Error(err))
			return err
		}
	}
	base.Logger.Info(fmt.Sprintf("Jenkins buildUrl: %s%d", task.Raw.Task.URL, buildID))
	base.Logger.Info(fmt.Sprintf("Jenkins buildNumber: %d", buildID))
	jobBuild, err := getBuild(ctx, buildID, jenkinsClient, job)
	if err != nil {
		base.Logger.Error(fmt.Sprintf("get jenkins build detail err:%v", err))
		return err
	}
...
func getBuild(ctx context.Context, buildID int64, jenkins *gojenkins.Jenkins, job *gojenkins.Job) (*gojenkins.Build, error) {
	...
	if err != nil {
		base.Logger.Error(fmt.Sprintf("url parse err:%v", err))
		return nil, err
	}
	...
	if err != nil {
		base.Logger.Error(fmt.Sprintf("build poll err:%v", err))
		return nil, err
	}
	if status == 200 {
		return &build, nil
